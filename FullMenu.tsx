...
export default function FullMenu({ onBack, orderLink, onAddToCart }: FullMenuProps) {
  return (
    <motion.div 
    ...
    >
      ...
      <div className="sticky top-0 z-50 bg-[#001233]/90 backdrop-blur-md border-b border-white/10 px-4 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <h1 className="font-display text-2xl sm:text-3xl tracking-tight uppercase">OUR MENU</h1>
          <div className="flex items-center gap-3">
            <button 
              onClick={onBack}
              className="hidden sm:block text-xs font-bold uppercase tracking-widest text-blue-300 hover:text-white"
            >
              Home
            </button>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 pt-8">
                  ...
                  <div key={i} className="group relative flex justify-between items-start border-b border-white/5 pb-4 hover:border-blue-500/30 transition-colors">
                    ...
                    <div className="text-right flex flex-col items-end gap-2">
                      ...
                      <button 
                        ...
                        className="p-2 bg-white/5 hover:bg-blue-600 rounded-lg transition-all active:scale-95 flex items-center gap-2"
                      >
                        <ShoppingCart className="w-4 h-4" />
                        <span className="text-[10px] font-bold uppercase tracking-widest hidden group-hover:inline">Add</span>
                      </button>
                    </div>
                  </div>
                ))}
        ...
        <div className="mt-24 text-center border-t border-white/10 pt-12">
          ...
          <button 
            onClick={onBack}
            className="bg-blue-900 hover:bg-blue-800 text-white px-12 py-6 rounded-2xl text-2xl font-black uppercase tracking-widest shadow-2xl transition-all active:scale-95"
          >
            Back to Home
          </button>
        </div>
